...
def rna_write_compressed_fastq_volume(data: List[Tuple[str, str, str, Tuple[bytes, bytes, int]]], 
...
def rna_write_compressed_fastq_with_volumes(data: List[Tuple[str, str, str, Tuple[bytes, bytes, int]]], 
                                          output_file: str,
                                          num_volumes: int,
                                          compress_plus: bool = False,
                                          preset: int = 3,
    ...
    for i, volume_data in enumerate(volumes, 1):
        rna_write_compressed_fastq_volume(
            volume_data,
            output_file,
            i,
            num_volumes,
...
def rna_write_compressed_fastq(data: List[Tuple[str, str, str, Tuple[bytes, bytes, int]]], 
                             output_file: str, 
                             compress_plus: bool = False, 
                             preset: int = 3, 
                             is_plant: bool = False,
    ...
    if num_volumes is not None:
        rna_write_compressed_fastq_with_volumes(data, output_file, num_volumes, compress_plus, preset, is_plant)
    else:
        annotations = [item[0] for item in data]
        annotation_patterns, variable_parts = compress_annotation(annotations)
...
def rna_read_compressed_file(file_path: str) -> Tuple[str, Union[List[Tuple[str, str]], List[Tuple[str, str, str, Tuple[bytes, bytes, int]]]]]:
    if file_path.endswith('.001'):
        return rna_read_compressed_volumes(file_path)
...
def rna_write_compressed_fasta_volume(data: List[Tuple[str, str]], 
                                    base_output_file: str,
                                    volume_number: int,
                                    total_volumes: int,
                                    preset: int = 3,
...
def rna_write_compressed_fasta_with_volumes(data: List[Tuple[str, str]], 
                                          output_file: str,
                                          num_volumes: int,
                                          preset: int = 3,
                                          is_plant: bool = False):
